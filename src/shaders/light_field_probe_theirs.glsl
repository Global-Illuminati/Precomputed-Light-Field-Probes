...
TraceResult highResolutionTraceOneRaySegment
   (in LightFieldSurface lightFieldSurface,
    ...
    float texCoordStep = (/*TODO: OPTIMIZE!*/vec2(1.0) / vec2(textureSize(lightFieldSurface.distanceProbeGrid, 0))).x * (texCoordDistance / maxComponent(abs(texCoordDelta)));
        ...
        if (abs(cosTheta) > 0.9999) {
            ...
            if (cosTheta > 0.0) {
                ...
                float distanceFromProbeToSurface = texelFetch(lightFieldSurface.distanceProbeGrid,
                    ivec2(vec2(textureSize(lightFieldSurface.distanceProbeGrid, 0)) * startTexCoord), 0).r;
                // TODO: Texture Arrays!
                //float distanceFromProbeToSurface = texelFetch(lightFieldSurface.distanceProbeGrid,
                //    ivec3(textureSize(lightFieldSurface.distanceProbeGrid, 0).xy * startTexCoord, probeIndex), 0).r;
    ...
    for (float d = 0.0f; d <= texCoordDistance; d += texCoordStep) {
        ...
        float distanceFromProbeToSurface = texelFetch(lightFieldSurface.distanceProbeGrid,
            ivec2(vec2(textureSize(lightFieldSurface.distanceProbeGrid, 0)) * texCoord), 0).r;
        // TODO: Texture Arrays!
        //float distanceFromProbeToSurface = texelFetch(lightFieldSurface.distanceProbeGrid,
        //    ivec3(textureSize(lightFieldSurface.distanceProbeGrid, 0).xy * texCoord, probeIndex), 0).r;
        ...
        if (maxDistFromProbeToRay >= distanceFromProbeToSurface) {
            ...
            // TODO: I skipped the octDecode since we just store the normals in world space, is that right?!
            vec3 normal = texelFetch(lightFieldSurface.normalProbeGrid, ivec2(vec2(textureSize(lightFieldSurface.distanceProbeGrid, 0)) * texCoord), 0).rgb * vec3(2.0) + vec3(1.0);
            // TODO: Texture Arrays!
            //vec3 normal = octDecode(texelFetch(lightFieldSurface.normalProbeGrid, ivec3(textureSize(lightFieldSurface.distanceProbeGrid, 0).xy * texCoord, probeIndex), 0).xy * lightFieldSurface.normalProbeGrid.readMultiplyFirst.xy + lightFieldSurface.normalProbeGrid.readAddSecond.xy);
...
bool lowResolutionTraceOneSegment
   (in LightFieldSurface lightFieldSurface,
    ...
    Vector2 lowResSize    = vec2(textureSize(lightFieldSurface.lowResolutionDistanceProbeGrid, 0).xy);
    Vector2 lowResInvSize = (/*TODO: OPTIMIZE!*/vec2(1.0) / vec2(textureSize(lightFieldSurface.lowResolutionDistanceProbeGrid, 0))).xy;
    ...
    float maxTexCoordDistance = squaredLength(segmentEndTexCoord - texCoord);
    ...
    for (Point2 P = P0; ((P.x * sign(delta.x)) <= end); ) {
        ...
        if (squaredLength(endHighResTexCoord - texCoord) > maxTexCoordDistance) {
...
TraceResult traceOneRaySegment
   (in LightFieldSurface lightFieldSurface,
    ...
    while (true) {
        ...
        if (dot(texCoordRayDirection, segmentEndTexCoord - endTexCoord) <= (/*TODO: OPTIMIZE!*/vec2(1.0) / vec2(textureSize(lightFieldSurface.distanceProbeGrid, 0))).x) {
            ...
            texCoord = endTexCoord + texCoordRayDirection * (/*TODO: OPTIMIZE!*/vec2(1.0) / vec2(textureSize(lightFieldSurface.distanceProbeGrid, 0))).x * 0.1;
        }
...
bool trace(LightFieldSurface lightFieldSurface, Ray worldSpaceRay, inout float tMax, out Point2 hitProbeTexCoord, out ProbeIndex hitProbeIndex, const bool fillHoles) {
    ...
    if ((hitProbeIndex == -1) && fillHoles) {
        ...
        float probeDistance = texelFetch(lightFieldSurface.distanceProbeGrid, ivec2(hitProbeTexCoord * vec2(textureSize(lightFieldSurface.distanceProbeGrid, 0).xy)), 0).r;
        // TODO: Texture Arrays!
        //float probeDistance = texelFetch(lightFieldSurface.distanceProbeGrid, ivec3(ivec2(hitProbeTexCoord * textureSize(lightFieldSurface.distanceProbeGrid, 0).xy), hitProbeIndex), 0).r;
